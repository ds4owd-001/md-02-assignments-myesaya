...
#getting the gapmider dataset and keeping it as an object
gapminder<-gapminder

#glimpse data
glimpse(gapminder)
 
#filter gapminder dataset and have a subset for year 2007, and assign the result to a new object
...
5.  What are the data types of the three variables used for aesthetic mappings?
    ...
    [-Data types used in aes:]{.underline}

    .continent for X (factor)

    .lifeExp for Y (integer)

    .continent for legend (factor)

    ```{r}
    #| echo: false
    tags$a(href = "https://www.perplexity.ai/search/8ba31702-07eb-4002-a374-23bc8811c08c?s=u", "Click here to visit the code that removes outliers in R boxplot made with ggplot package")
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    The geom_bar works with a dataset in its original form, and it automatically calculates counts and does not need a Y variable to be defined, probably making it an easy and less time-consuming method. The geom geom_col is a bit complicated and requires more code. It will need both X and Y variables to be defined and also needs the data to be in a summarised form, whereby one bar is only a single row in the dataset.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    The first number represents the minimum, the second number is the maximum, and the third number indicates the incremental value onto which breaks will be set. The larger the breaks, the larger the bars, the larger the width of the bar (the last number represents bin width)

    for

    see @fig-given-seq for a histogram with seq (40,85,2.5)
    ...
    see @fig-changed-seq for a histogram with seq (40,80,10)

```{r}
#| echo: false
#| label: fig-changed-seq
#| fig-cap: "Histogram B."
ggplot(gapminder_2007,
       aes(x=lifeExp,
           fill = continent))+
geom_histogram(col = "grey30", breaks = seq(40, 80, 10))
```
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    There are colours for the different countries contained in the gapminder package.
